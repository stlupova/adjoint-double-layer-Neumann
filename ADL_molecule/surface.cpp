...
void find_Third_Coordinate(int i, int n, int i1, int i2, int i3, double z0, const vector<double>& p1,
  ...
  int n_max = 100;
  for (int j=0; j<n; j++) {
    // start Newton's iteration using z0 value as initial guess
    vector<double> xn(3,0);
    xn[i1] = p1[j];
    xn[i2] = p2[j];
    xn[i3] = z0;
    double phi_n = phi(xn);
    double Dphi_n = Dphi(i3,xn);
    int n = 0;
    while ((abs(phi_n) > tol) && (abs(Dphi_n) > tol) && (n < n_max)) {
      xn[i3] -= phi_n/Dphi_n; 
      n += 1;
    ...
    if ((n >= n_max) || (abs(Dphi_n) <= tol)) {
      cout << "Newton's didn't converge for finding third coordinate..." << endl;
    }
    else {
      p3[j] = xn[i3];
    }
